...
class Module extends Controller
{
    ...
    public function appSelectModule($application_id, $module_id){
        if(!isset($application_id)) return APIResponse::FAIL(["Không thể truy cập ứng dụng"]);
        if(!isset($module_id)) return APIResponse::FAIL(["Không tìm thấy tính năng cần thêm"]);  
        $findModule = ModuleModel::find($module_id);
        if(!isset($findModule)) return APIResponse::FAIL(["Không tìm thấy tính năng yêu cầu"]);
    ...
    public function appUnSelectModule($application_id, $module_id){
        if(!isset($application_id)) return APIResponse::FAIL(["Không thể truy cập ứng dụng"]);
        if(!isset($module_id)) return APIResponse::FAIL(["Không tìm thấy tính năng cần xoá"]);  
        $findModule = ModuleModel::find($module_id);
        if(!isset($findModule)) return APIResponse::FAIL(["Không tìm thấy tính năng yêu cầu"]);
        $findModuleInApplication = SelectModuleModel::where("application_id", '=', $application_id)->where("module_id", '=', $module_id)->first();

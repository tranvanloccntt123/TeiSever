...
class Application extends Controller
{
    ...
    public function create(Request $request){
        ...
        if($validator->fails()) return APIResponse::FAIL($validator->errors(), $this->isApi);
        ApplicationModel::insert(['name' => $request->name, 'price' => $request->has("price")? $request->price : 0, 'type_id' => $request->type_id]);
        return APIResponse::SUCCESS("Ứng dụng đã được đăng kí", $this->isApi);
    }
    ...
    public function delete(Request $request){
        ...
        return APIResponse::SUCCESS("Ứng dụng đã được xóa", $this->isApi);
    }

    public function detail($id){
        $application = ApplicationModel::find($id);
        if(!isset($application)) return APIResponse::FAIL(['application' => ["Không thể truy cập ứng dụng"]], $this->isApi);
        return APIResponse::DATA($application);
    }

    public function edit(Request $request){
        if(!$request->has("id")) return APIResponse::FAIL(['application' => ["Không thể truy cập ứng dụng"]], $this->isApi);
